...
namespace FantasyBaseball.PlayerDatabaseService.Services.UnitTests
{
    public class PlayerEntityMergerServiceTest
    {
        ...
        private static BaseballPlayer BuildPlayer(int value, PlayerType type) =>
            new BaseballPlayer 
            {
                ...
                {
                    value == 10 ? BuildBattingStats(StatsType.YTD) : new BattingStats { StatsType = StatsType.YTD },
                    value != 10 ? BuildBattingStats(StatsType.PROJ) : new BattingStats { StatsType = StatsType.PROJ }
                },
                PitchingStats = new List<PitchingStats> 
                {
                    value == 10 ? BuildPitchingStats(StatsType.YTD) : new PitchingStats { StatsType = StatsType.YTD },
                    value != 10 ? BuildPitchingStats(StatsType.PROJ) : new PitchingStats { StatsType = StatsType.PROJ }
                }
            };
        ...
        private static void ValidatePlayer(int value, BaseballPlayer expected, PlayerEntity actual)
        {
            ...
            ValidatePlayerBattingStats(expected.BattingStats.ToDictionary(s => s.StatsType, s => s)[value == 10 ? StatsType.YTD : StatsType.PROJ], actual.BattingStats.First());
            ValidatePlayerPitchingStats(expected.PitchingStats.ToDictionary(s => s.StatsType, s => s)[value == 10 ? StatsType.YTD : StatsType.PROJ], actual.PitchingStats.First());
        }
